...
import be.tzbob.circleroyale.Keyboard.Key
...
class Keyboard {
  private[this] def keyEvSource(event: String): ClientEventSource[Key] =
    ClientEvent.sourceWithEngineEffect[Key] { (fire: Key => Unit) =>
      @client val _ = {
        import org.scalajs.dom
        import org.scalajs.dom.raw.KeyboardEvent
        dom.window.addEventListener[KeyboardEvent](event, ev => {
          if (!ev.repeat) fire(ev.key)
        })
      }
    }

  private[this] val keyDownSource: ClientEventSource[Key] = keyEvSource(
    "keydown")
  private[this] val keyUpSource: ClientEventSource[Key] = keyEvSource("keyup")

  val keyDown: ClientEvent[Key] = keyDownSource
  val keyUp: ClientEvent[Key]   = keyUpSource

  def keyDown(key: Key): ClientEvent[Unit] = {
    keyDown.dropIf(_ != key).map(_ => ())
  }

  def isKeyDown(key: Key): ClientDBehavior[Boolean] = {
